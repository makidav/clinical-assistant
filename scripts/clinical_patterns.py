...
def _fuzzy_find(query: str, mapping: dict, name_field: str) -> tuple[str, dict] | tuple[None, None]:
    ...
    PATCHED (clinical-assistant v6.3): name_field may hold either a string
    (SYNDROMES['name'], RED_FLAGS['symptom']) or a list of strings
    (OCCUPATIONAL_EXPOSURES['diseases']). The upstream version called .lower()
    unconditionally and crashed with AttributeError on list-valued fields,
    making `--type occupational` unusable. Now normalises both shapes.
    """
    q = query.lower()
    for key, entry in mapping.items():
        if q in key:
            return key, entry
        field = entry.get(name_field)
        if isinstance(field, str):
            haystack = field.lower()
        elif isinstance(field, (list, tuple)):
            haystack = " | ".join(str(x) for x in field).lower()
        else:
            continue
        if q in haystack:
            return key, entry
