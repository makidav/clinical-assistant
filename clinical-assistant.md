...
description: >
  A self-contained Virtual Clinical Team that runs a 7-phase workflow — intake, evidence
  search, GRADE validation, multidisciplinary board deliberation, treatment plan, final
  report, and QA — with the whole team embedded inside this one skill (no other skills to
  install). Trigger when the user presents symptoms, a diagnosis, or treatment questions, or
  says: "I have these symptoms", "necesito un plan de tratamiento", "analyze this case",
  "escribe un informe clínico", "clinical board", "junta clínica", or "clinical-assistant".
  Fully bilingual English/Spanish (interface, evidence, and output). Uses PubMed and other
  medical databases when connected, degrading gracefully to web search when they are not.
  Produces research/decision-support DRAFTS only — never a diagnosis, prescription, or a
...
metadata:
  ...
  self-contained: true
  integrates: 27 embedded clinical, scientific, deliberation, and documentation capabilities
  language: Fully bilingual (English + Spanish) — interface, evidence sourcing, and output
---
